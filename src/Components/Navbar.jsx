...
const Navbar = ({ darkMode, toggleDarkMode }) => {
  ...
  return (
    <header className="flex justify-end items-center h-14 max-w-[100%] z-50 mx-auto px-4 text-white color-font absolute right-0 left-0 top-4  max-md:justify-end ">
      ...
      <div
        onClick={handleNav}
        className="md:hidden flex justify-center backdrop-blur-lg fixed text-gray-400"
      >
      ...
      <div
        className={
          nav
            ? "fixed left-0 top-0 w-[50%]  items-center h-full z-50 backdrop-blur-3xl  "
            : "fixed left-[-100%]"
        }
      >
        <ul className="uppercase p-4 ">
          {menuOpciones.map((menu) => (
            <ListMenuMobile
              opcion={menu}
              currentPage={currentPage}
              setCurrentPage={setCurrentPage}
